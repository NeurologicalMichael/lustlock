...
function MountainBg() {
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      <LinearGradient
        colors={['#080118', '#130630', '#200E50', '#160830', '#08080F']}
      ...
      <Svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={StyleSheet.absoluteFill}>
        ...
        {[[28,52],[72,38],[118,82],[188,30],[248,65],[308,42],[352,80],[55,115],
          ...
          <Circle key={i} cx={x} cy={y} r={i%4===0?1.4:0.9} fill={`rgba(255,255,255,${i%3===0?0.7:0.4})`}/>
        ))}
        <Path d={`M${-10},${H} L${-10},${H*.58} L${W*.12},${H*.35} L${W*.28},${H*.50} L${W*.5},${H*.26} L${W*.66},${H*.42} L${W*.82},${H*.31} L${W+10},${H*.48} L${W+10},${H} Z`} fill="rgba(48,18,110,0.50)"/>
        <Path d={`M${-10},${H} L${-10},${H*.65} L${W*.18},${H*.44} L${W*.38},${H*.52} L${cx},${H*.175} L${W*.62},${H*.50} L${W*.82},${H*.42} L${W+10},${H*.60} L${W+10},${H} Z`} fill="rgba(16,6,42,0.88)"/>
        <Path d={`M${-10},${H} L${-10},${H*.74} Q${W*.22},${H*.64} ${cx},${H*.70} Q${W*.78},${H*.76} ${W+10},${H*.66} L${W+10},${H} Z`} fill="rgba(8,4,20,0.92)"/>
        <Circle cx={cx} cy={crossMid} r={32} fill="url(#cg)"/>
        <Rect x={cx-2.2} y={crossTop} width={4.4} height={crossBottom-crossTop} rx={1.5} fill="rgba(240,165,55,0.92)"/>
        <Rect x={cx-14} y={crossMid-2.2} width={28} height={4.4} rx={1.5} fill="rgba(240,165,55,0.92)"/>
      </Svg>
    </View>
  );
}
...
function GoldButton({ children, onPress, outline = false, style }: {
  children: React.ReactNode; onPress: () => void; outline?: boolean; style?: object;
}) {
  return (
    <TouchableOpacity activeOpacity={0.8} onPress={onPress}
      style={[outline ? s.btnOutline : s.btnGold, style]}>
...
export default function Onboarding() {
  ...
  const { setOnboardingData, setUserId } = useAppStore();
  ...
  useEffect(() => {
    if (step === 19) {
      ...
      const t = setInterval(() => {
        i++;
        setChecklistDone(i);
        if (i >= CHECKLIST.length) { clearInterval(t); setTimeout(next, 900); }
      }, 620);
  ...
  const selectOption = useCallback((qIndex: number, optIndex: number) => {
    ...
    cardScale.value = withSequence(
      withTiming(1.02, { duration: 100, easing: Easing.out(Easing.ease) }),
    ...
    <View style={s.centeredFlex}>
      <View style={s.landingIconWrap}>
        <Svg width={70} height={90} viewBox="0 0 70 90">
          <Rect x={31} y={2}  width={8}  height={60} rx={3} fill={Colors.gold}/>
          <Rect x={10} y={22} width={50} height={8}  rx={3} fill={Colors.gold}/>
        </Svg>
      </View>
      ...
      <View style={s.chipRow}>
        {['17-Question Diagnostic','Personalized 60-Day Plan','Daily Accountability'].map(c=>(
          <View key={c} style={s.featureChip}><Text style={s.featureChipText}>{c}</Text></View>
        ))}
      </View>
      <View style={{width:'100%',marginTop:32}}>
        <GoldButton onPress={next}>Start Your Assessment →</GoldButton>
      </View>
      <Text style={s.alreadyText}>Takes less than 3 minutes</Text>
    </View>
  );
  ...
  const renderEmail = () => (
    <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={s.centeredFlex}>
      <View style={s.inputCard}>
        ...
        <TextInput style={s.input} placeholder="your@email.com" placeholderTextColor="rgba(255,255,255,0.30)"
  ...
  const renderName = () => (
    <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={s.centeredFlex}>
      <View style={s.inputCard}>
        ...
        <TextInput style={s.input} placeholder="Your first name" placeholderTextColor="rgba(255,255,255,0.30)"
      ...
      <View style={s.inputCard}>
        ...
        <View style={s.pwWrap}>
          <TextInput style={[s.input,{flex:1,marginBottom:0}]} placeholder="Password"
            placeholderTextColor="rgba(255,255,255,0.30)" secureTextEntry={!showPw}
  ...
  const renderNotifications = () => (
    <View style={s.centeredFlex}>
      ...
      <Text style={s.darkSub}>Notifications keep you on track when it matters most.</Text>
      <View style={s.notifBullets}>
        {[['Morning check-in','Start each day with intention'],
          ['Streak reminders','Never lose your momentum'],
          ['Emergency alert','One tap when urges strike']].map(([title,desc])=>(
          <View key={title} style={s.notifBullet}>
            <View style={s.notifDot}/>
            <View style={{flex:1}}>
              <Text style={s.notifBulletTitle}>{title}</Text>
      ...
      <GoldButton onPress={async()=>{
        try{await ExpoNotifications.requestPermissionsAsync();}catch{}
        next();
  ...
  const renderPremium = () => (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.premiumScroll}>
      <Text style={s.darkTitle}>Unlock Your Full{'\n'}60-Day Plan</Text>
      <Text style={s.darkSub}>Everything you need to win this battle, structured and proven.</Text>
      <TouchableOpacity activeOpacity={0.85} onPress={()=>setPlan('annual')} style={[s.planCard,plan==='annual'&&s.planCardSel]}>
        <View style={s.planCardHeader}><View style={s.mostPopularBadge}><Text style={s.mostPopularText}>MOST POPULAR</Text></View></View>
        <Text style={s.planName}>Annual Plan</Text>
        <Text style={s.planPrice}>$49.99<Text style={s.planPer}> / year</Text></Text>
        <Text style={s.planEquiv}>Just $4.17/month · Save 58%</Text>
        {plan==='annual'&&<View style={s.planCheck}><Text style={{color:Colors.bg,fontFamily:'Cinzel_700Bold',fontSize:11}}>✓ SELECTED</Text></View>}
      </TouchableOpacity>
      <TouchableOpacity activeOpacity={0.85} onPress={()=>setPlan('monthly')} style={[s.planCard,plan==='monthly'&&s.planCardSel,{marginTop:10}]}>
        <Text style={s.planName}>Monthly Plan</Text>
        <Text style={s.planPrice}>$9.99<Text style={s.planPer}> / month</Text></Text>
        <Text style={s.planEquiv}>Cancel anytime</Text>
        {plan==='monthly'&&<View style={s.planCheck}><Text style={{color:Colors.bg,fontFamily:'Cinzel_700Bold',fontSize:11}}>✓ SELECTED</Text></View>}
      </TouchableOpacity>
      <Text style={s.trialBadge}>7-DAY FREE TRIAL INCLUDED</Text>
      <GoldButton onPress={next} style={{marginTop:16}}>Start Free Trial →</GoldButton>
      <TouchableOpacity activeOpacity={0.85} onPress={next} style={s.applePayBtn}>
      ...
      <Text style={s.privacyNote}>Billed through the App Store · Cancel anytime</Text>
    </ScrollView>
  );
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.readyScroll}>
      <View style={s.readyIconWrap}>
        <Svg width={80} height={80} viewBox="0 0 80 80">
          <Circle cx={40} cy={40} r={36} stroke={Colors.gold} strokeWidth={3} fill="rgba(240,112,32,0.10)"/>
    ...
    <View style={s.centeredFlex}>
      ...
      <View style={s.profileCard}>
        <View style={s.profileAvatar}>
          <Svg width={52} height={52} viewBox="0 0 52 52">
            <Circle cx={26} cy={20} r={10} stroke={Colors.gold} strokeWidth={2} fill="rgba(240,112,32,0.12)"/>
    ...
    <View style={s.centeredFlex}>
      <View style={s.finalGlow}>
        <Svg width={90} height={110} viewBox="0 0 90 110">
          <Circle cx={45} cy={55} r={42} fill="rgba(240,112,32,0.08)"/>
  ...
  const renderNonQuizStep = () => {
    ...
    if (step === 20) return renderEmail();
    if (step === 21) return renderName();
    if (step === 22) return renderPassword();
    if (step === 23) return renderNotifications();
    if (step === 24) return renderScreenTimePermission();
    if (step === 25) return renderTestimonials();
    if (step === 26) return renderStats();
    if (step === 27) return renderPremium();
    if (step === 28) return renderReady();
    if (step === 29) return renderScripture();
    if (step === 30) return renderProfilePreview();
  ...
  if (isQuiz) {
    const q = QUIZ[qIdx];
    const progressPct = Math.round((step - 1) / QUIZ_COUNT * 100);
      ...
      <View style={s.root}>
        ...
        <View style={[s.quizTopBar, { paddingTop: insets.top + 8 }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.progressLabel}>{progressPct}% complete</Text>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  backChevronLight: {
    fontSize: 30, color: Colors.white, lineHeight: 34,
  },
  ...
  optionBulletSel: { backgroundColor: '#007AFF', borderColor: '#007AFF' },
  optionCheck: { fontSize: 12, color: '#fff', fontFamily: 'Cinzel_700Bold' },
  optionText: {
    fontFamily: 'CrimsonPro_400Regular',
    fontSize: 14, color: '#2D1060', flex: 1, lineHeight: 20,
  },
  optionTextSel: { color: '#003E99', fontFamily: 'CrimsonPro_600SemiBold' },

  // ── Landing ──
  landingIconWrap: { marginBottom: 16 },
  logoText: {
    fontFamily: 'CinzelDecorative_900Black',
    fontSize: 32, letterSpacing: 5, color: Colors.gold, textAlign: 'center',
  },
  tagline: {
    fontFamily: 'CrimsonPro_400Regular_Italic',
    fontSize: 15, color: Colors.white3, marginTop: 8, textAlign: 'center',
  },
  divider: { width: 40, height: 1, backgroundColor: 'rgba(240,112,32,0.3)', marginVertical: 22 },
  ...
  featureChipText: {
    fontFamily: 'Cinzel_600SemiBold', fontSize: 9, letterSpacing: 1, color: Colors.gold,
  },
  alreadyText: {
    fontFamily: 'CrimsonPro_400Regular', fontSize: 12,
    color: Colors.white3, marginTop: 14, textAlign: 'center',
  },
  ...
  btnGoldText: {
    fontFamily: 'Cinzel_700Bold', fontSize: 13, letterSpacing: 1.5, color: '#0A0520',
  },
  btnOutline: {
    borderWidth: 1.5, borderColor: Colors.goldBorder, borderRadius: 14,
    paddingVertical: 14, alignItems: 'center', justifyContent: 'center',
  },
  btnOutlineText: {
    fontFamily: 'Cinzel_700Bold', fontSize: 13, letterSpacing: 1.5, color: Colors.gold,
  },
  skipText: {
  ...
  darkSub: {
    ...
    color: Colors.white2, textAlign: 'center', lineHeight: 22,
  },
  ...
  analysisHint: {
    fontFamily: 'CrimsonPro_400Regular', fontSize: 12,
    color: Colors.white3, textAlign: 'center', marginTop: 10,
  },
  ...
  checklistCheckmark: { fontSize: 12, color: Colors.bg, fontFamily: 'Cinzel_700Bold' },
  checklistText: {
    fontFamily: 'CrimsonPro_400Regular', fontSize: 14, color: Colors.white3, flex: 1,
  },

  // ── Input screens ──
  inputCard: {
    width: '100%', backgroundColor: 'rgba(20,20,30,0.88)',
  ...
  inputCardSub: {
    fontFamily: 'CrimsonPro_400Regular', fontSize: 13,
    color: Colors.white3, lineHeight: 20, marginBottom: 18,
  },
  input: {
  ...
  privacyNote: {
    ...
    color: Colors.white3, textAlign: 'center', marginTop: 14,
  },
  ...
  notifBulletDesc: { fontFamily: 'CrimsonPro_400Regular', fontSize: 13, color: Colors.white3, marginTop: 2 },

  // ── Testimonials ──
  testimonialCard: {
    backgroundColor: 'rgba(20,14,40,0.90)', borderWidth: 1.5, borderColor: Colors.goldBorder,
  ...
  testimonialBadgeText: { fontFamily: 'Cinzel_700Bold', fontSize: 9, letterSpacing: 2, color: Colors.bg },
  testimonialQuote: {
    fontFamily: 'CrimsonPro_400Regular_Italic', fontSize: 15, color: Colors.white2, lineHeight: 26,
  },
  testimonialName: {
  ...
  testimonialDot: { width: 7, height: 7, borderRadius: 3.5, backgroundColor: 'rgba(255,255,255,0.20)' },
  ...
  statCard: {
    flex: 1, minWidth: '45%', backgroundColor: 'rgba(20,14,40,0.82)',
  ...
  statLabel: {
    fontFamily: 'CrimsonPro_400Regular', fontSize: 12,
    color: Colors.white3, textAlign: 'center', marginTop: 4, lineHeight: 17,
  },

  // ── Premium ──
  premiumScroll: { paddingTop: 8, paddingBottom: 24 },
  planCard: {
    backgroundColor: 'rgba(20,14,40,0.88)', borderWidth: 1.5,
    borderColor: Colors.border, borderRadius: 18, padding: 20,
  },
  planCardSel: { borderColor: Colors.gold, backgroundColor: 'rgba(255,215,0,0.08)' },
  planCardHeader: { marginBottom: 8 },
  mostPopularBadge: {
    alignSelf: 'flex-start', backgroundColor: Colors.gold,
    borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4,
  },
  mostPopularText: { fontFamily: 'Cinzel_700Bold', fontSize: 8, letterSpacing: 2, color: Colors.bg },
  planName: { fontFamily: 'Cinzel_700Bold', fontSize: 15, letterSpacing: 1, color: Colors.white, marginBottom: 6 },
  planPrice: { fontFamily: 'Cinzel_700Bold', fontSize: 26, letterSpacing: 0.5, color: Colors.gold },
  planPer: { fontFamily: 'CrimsonPro_400Regular', fontSize: 14, color: Colors.white3 },
  planEquiv: { fontFamily: 'CrimsonPro_400Regular', fontSize: 12, color: Colors.white3, marginTop: 2 },
  planCheck: {
    marginTop: 10, alignSelf: 'flex-start',
    backgroundColor: Colors.gold, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5,
  },
  trialBadge: {
    fontFamily: 'Cinzel_600SemiBold', fontSize: 10, letterSpacing: 2,
    color: Colors.gold, textAlign: 'center', marginTop: 16,
  },
  applePayBtn: {
    marginTop: 10, backgroundColor: '#1A1A1A', borderRadius: 14,
    paddingVertical: 16, alignItems: 'center', justifyContent: 'center',
    borderWidth: 1, borderColor: Colors.border,
  },
  applePayText: { fontSize: 17, color: Colors.white, fontWeight: '600' },
  ...
  readyMessageCard: {
    ...
    backgroundColor: 'rgba(240,112,32,0.10)', padding: 22,
  ...
  timelineDesc: { fontFamily: 'CrimsonPro_400Regular', fontSize: 12, color: Colors.white3, marginTop: 2 },

  // ── Scripture ──
  scriptureCard: {
    backgroundColor: 'rgba(16,6,42,0.82)', borderWidth: 1, borderColor: Colors.goldBorder,
    borderRadius: 20, padding: 26, width: '100%',
  },
  scriptureQuote: {
    fontFamily: 'CrimsonPro_400Regular_Italic', fontSize: 17,
    color: Colors.white2, lineHeight: 30, textAlign: 'center',
  },
  scriptureDivider: {
  ...
  // ── Profile preview ──
  profileCard: {
    backgroundColor: 'rgba(16,6,42,0.88)', borderWidth: 1.5, borderColor: Colors.goldBorder,
  ...
  profileStatLbl: { fontFamily: 'CrimsonPro_400Regular', fontSize: 11, color: Colors.white3, marginTop: 2 },

  // ── Final CTA ──
  finalGlow: {
    width: 110, height: 110, borderRadius: 55,
    backgroundColor: 'rgba(240,112,32,0.08)',
    borderWidth: 1.5, borderColor: Colors.goldBorder,
    alignItems: 'center', justifyContent: 'center', marginBottom: 24,
  },
  finalTitle: {
